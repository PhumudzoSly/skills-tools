...
async function listSkillPaths(options: {
  ...
  if (!response.ok) {
    if (response.status === 403 || response.status === 401) {
      throw new Error(
        `GitHub API error: ${response.status} ${response.statusText} - Possibly rate-limited or invalid token.`,
      );
    }
    return [];
  }
...
async function fetchRawFile(options: {
  ...
  const response = await options.fetchFn(rawUrl, { headers });
...
function computePathScore(options: {
  ...
  let score = 0;
  if (options.preferredPath && options.path === options.preferredPath)
    score += 100;
...
function computeMatchScore(options: {
  ...
  else if (requestedNameKey && parsedKey.includes(requestedNameKey))
    score += 15;
...
export async function readSkillMarkdown(options: {
  ...
  const maxCandidates = Math.max(
    1,
    Math.min(50, options.maxCandidatesToRead ?? 20),
  );
